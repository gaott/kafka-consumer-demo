...
public abstract class BaseWorker implements Runnable {
	...
	public void run() {
		...
		try {
			...
			try {
				...
				Log.f(String.format("[SAVE_MESSAGE_FAIL] - %s - %s", topic,
						message));
				Log.e(e1);
			}
			return;
	...
	public String saveMessage() throws DBException, JSONException {
		String key = String.format("%s_%s_%s", topic, this.getClass()
				.getSimpleName(), System.currentTimeMillis());

		leveldb.put(bytes(key),
				bytes(new JSONObject().put("topic", topic)
						.put("value", message).toString()));
		return key;
	}
